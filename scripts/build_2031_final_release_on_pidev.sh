...
echo "=== PiViewer 2031 final release build helper ==="
echo "Werkmap: ${WORK}"
...
echo
echo "=== Lokale mislukte wijzigingen opruimen ==="
...
echo
echo "=== main.py: raysnijder.nl web-update verwijderen ==="
python3 - <<'PY'
...
old = '''            # Web-update is een extra updatekanaal. USB-update houdt altijd prioriteit.
...
echo
echo "=== app/web_update.py verwijderen ==="
git rm -f app/web_update.py 2>/dev/null || rm -f app/web_update.py

echo
echo "=== web.py: titel uit config, standaard PiViewer final ==="
python3 - <<'PY'
from pathlib import Path
p = Path("app/web.py")
s = p.read_text(encoding="utf-8")

# Voeg app_title toe in layout() als dat nog niet aanwezig is.
...
echo
echo "=== config voorbeeld: final titel, geen web_update ==="
python3 - <<'PY'
import json
from pathlib import Path
...
echo
echo "=== install_dev.sh: configmigratie, web-update cleanup, reboot na GitHub-update ==="
python3 - <<'PY'
from pathlib import Path
...
cfg_path = Path("${CONFIG_DIR}/piviewer.json")
try:
...
if "reboot-after-github-update" not in s:
    ...
    print("install_dev.sh bevat al reboot-after-github-update")

p.write_text(s, encoding="utf-8")
PY

echo
echo "=== github_auto_update.py: reboot na succesvolle update ==="
python3 - <<'PY'
from pathlib import Path
...
if "reboot-after-github-update" not in s:
    ...
    print("github_auto_update.py bevat al reboot-after-github-update")
p.write_text(s, encoding="utf-8")
PY
...
git add -A VERSION app/main.py app/web.py app/github_auto_update.py scripts/install_dev.sh config/piviewer.example.json updates/index.json app/web_update.py scripts/build_2031_final_release_on_pidev.sh
...
if [ "${COMMIT_CODE}" -ne 0 ]; then
  echo "Geen commit gemaakt of commit mislukt. We proberen push alleen als lokale main voorloopt."
fi
